...
public class AttachmentDownloadServiceImpl implements AttachmentDownloadService {
    ...
    @Transactional
    @Override
    public ApiResponse<List<String>> download(List<Long> fileIds) {
        List<String> attachmentFilePaths = new ArrayList<>();
        for (Long fileId : fileIds) {
            String attachmentFilePath = attachmentRepository.findAbsolutePathById(fileId);
            attachmentFilePaths.add(attachmentFilePath);
        }
