...
public class InquiryAnswerServiceTest {
    ...
    void 문의_답변_등록_생성_성공_시() {
        ...
        ApiResponse<AnswerResponse.Response> response = inquiryAnswerService.create(request);

        assertEquals(answer.getContent(), response.getData().content());
    }

    @Test
    void 문의_답변_댓글_수정_성공_시() {
        ...
        ApiResponse<AnswerResponse.Response> response = inquiryAnswerService.update(1L, request);
    ...
    @Test
    void 문의_답변_댓글_단건_조회_성공_시() {
        long answerId = 1L;
        Answer answer = new Answer("답변 본문", AnswerType.APPLY_ANSWER);

        given(answerRepository.findById(answerId)).willReturn(Optional.of(answer));

        ApiResponse<AnswerResponse.Response> response = inquiryAnswerService.viewOne(answerId);

        assertEquals(answer.getContent(), response.getData().content());
    }

    @Test
    void 문의_답변_댓글_삭제_성공_시() {
        Answer answer = new Answer("답변 본문", AnswerType.APPLY_ANSWER);

        given(answerRepository.findById(anyLong())).willReturn(Optional.of(answer));

        ApiResponse<Void> response = inquiryAnswerService.delete(1L);

        assertEquals("문의 답변 삭제에 성공하였습니다.", response.getMessage());
    }
}
